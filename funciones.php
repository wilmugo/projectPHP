...
function editarCategoria($categoria, $descripcion, $id){
    $sentencia = "UPDATE categorias SET categoria = ?, descripcion = ? WHERE id = ?";
    $parametros = [$categoria, $descripcion, $id];
    return editar($sentencia, $parametros);
}

function obtenerCategoriaPorId($id){
    $sentencia = "SELECT * FROM categorias WHERE id = ?";
    $categoria = select($sentencia, [$id]);
    if($categoria) return $categoria[0];
}
...
function registrarCategoria($categoria, $descripcion ){
    $sentencia = "INSERT INTO categorias (categoria, descripcion) VALUES (?,?)";
    $parametros = [$categoria, $descripcion];
    return insertar($sentencia, $parametros);
}
...
function registrarProducto($codigo, $nombre, $compra, $venta, $existencia, $categoria){
    $sentencia = "INSERT INTO productos(codigo, nombre, compra, venta, existencia, categorias_id) VALUES (?,?,?,?,?,?)";
    $parametros = [$codigo, $nombre, $compra, $venta, $existencia, $categoria];
    return insertar($sentencia, $parametros);
}
